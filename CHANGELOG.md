...
### Fixed
- **Unreadable mobile download UX** (schoedel-learn/barry-ai#201): tapping a download control on Android or iOS gave no immediate feedback and a failed save to Downloads used to slide silently into a share sheet, leaving the user unsure whether anything happened. Tapping a download now shows an immediate busy state (spinner, disabled control, `accessibilityState.busy`) with duplicate-tap protection; Android keeps one-tap **Save to Downloads**, iOS opens the native document export sheet ("Save file…") instead of a generic share sheet, and a failed Downloads write reports the failure and offers **Choose location…** / **Share file** instead of falling back automatically. Confirmations describe the real outcome ("Saved to Downloads" / "File saved"), a cancelled picker is neutral, and a share sheet is left to speak for itself. Backed by `lib/download-plan.ts` (explicit `DownloadIntent`/`DownloadOutcome` mapping), `lib/document-save.ts` (native Save As via `@react-native-documents/picker`), and the shared `lib/use-file-download.ts` hook used by both the recording screen and Files.
...
- **Dead Fireworks model id (silent latency regression)**: the Fireworks primary slot for regular conversions requested `accounts/fireworks/models/deepseek-v4-flash`, which Fireworks answers with HTTP 404 ("Model not found, inaccessible, and/or not deployed"). Every regular conversion therefore paid a failed round-trip before falling through to official DeepSeek, so the lower first-token latency this slot exists for was never actually in effect. The routing default (`server/conversion-model-routing.ts`) and the client default (`server/openai-client.ts`) now use `accounts/fireworks/models/deepseek-v4p1-flash` (verified HTTP 200).

### Changed
- **Version bump**: Android versionCode 6, versionName 1.0.9.
- **DeepSeek models standardized on `deepseek-flash`**: every DeepSeek route now requests `deepseek-flash` (DeepSeek V4.1 Flash, released 2026-09-10) for both the regular and advanced buckets, replacing the `deepseek-v4-flash` and `deepseek-v4-pro` identifiers; `AI_DEEPSEEK_FLASH_MODEL` and `AI_DEEPSEEK_PRO_MODEL` in `scripts/deploy.sh` carry the new identifier. The user-selectable catalog entries were renamed to match (`deepseek_flash`, `deepseek_flash_fireworks`, `deepseek_advanced`) and the retired ids are still accepted on read, so a saved model preference keeps working instead of failing validation and resetting to the default.
